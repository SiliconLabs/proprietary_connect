/***************************************************************************//**
 * @file flex-callbacks.c
 * @brief Multi-Region Sensor Example
 * @version 1.0.1
 *******************************************************************************
 * # License
...
#define Region_902	0
#define Region_868	21
#define num_Regions	2

REGION Regions[num_Regions] = {{Region_902, "US"},	// Populate region information
							   {Region_868, "EU"}};
...
void joinCommand(void)
{
  ...
  parameters.radioChannel = emberUnsignedCommandArgument(0) + Regions[current_Region].baseChannel;
...
void infoCommand(void)
{
  ...
  emberAfCorePrintln("         Region: %p", Regions[current_Region].regionName);	// Display current region info
  emberAfCorePrintln("        channel: %d", (uint16_t)emberGetRadioChannel() - Regions[current_Region].baseChannel); // Apply base channel offset for region change
...
{
  EmberStatus status;
  uint8_t channelToScan = emberUnsignedCommandArgument(0) + Regions[current_Region].baseChannel;
...
void getRegionCommand(void){
	emberAfCorePrintln("Current Region is: %p", Regions[current_Region].regionName);
}

void setRegionCommand(void){
	uint8_t setRegion = emberUnsignedCommandArgument(0);
	if(emberNetworkState() != EMBER_NO_NETWORK){
		emberAfCorePrintln("Can not change regions while part of a network");
	} else {
		if(setRegion < num_Regions){
			current_Region = setRegion;
			emberAfCorePrintln("Current Region is: %p", Regions[current_Region].regionName);
		} else {
			emberAfCorePrintln("Region set failed");
		}
	}
}
/*~~~~~~~~~~~~~~~~~End User Code~~~~~~~~~~~~~~~~*/
